...
```powershell
Set-SPOTenant [-AllowMediaTranscription <Boolean>]
...
- True (default) - Videos can have transcripts generated on demand.
- False - Videos cannot have transcripts generated by Stream in any scenario. Note that there are other scenarios where transcripts could still exist.  
...
> [!NOTE]
> This setting is not used anymore with IE retired and the parameter would be removed soon. Users need to select "Yes" when prompted for "Stay signed in?" at the time of sign-in for "View in File Explorer" to work with Microsoft Edge. 
